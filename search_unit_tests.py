...
class SearchUnitTests(unittest.TestCase):
    """
    Error Diagnostic code courtesy
    ...
    If you failed on  because of non-optimal path, make sure you pass
    all the local tests.
    Change test_count=-1 if you failed the path test on , it will run
    tests on atlanta until it finds a set of points that fail.

    If you failed on  because of your explored set is too large,
    ...
    def reference_path(self, graph, src_node, dst_node, weight='weight'):
        ...
        if src_node == dst_node:
            ...
            path = networkx.shortest_path(
                graph, src_node, dst_node, weight=weight)
    ...
    def test_bfs_romania(self):
        ...
        for src in keys:
            for dst in keys:
                ...
                ref_len, ref_path = self.reference_bfs_path(
                    self.romania, src, dst)
                ...
                if src != dst:  # we want path == [] if src == dst
                    self.assertTrue(len(path) == len(
                        ref_path), msg="Path is too long. Real path: %s, your path: %s" % (ref_path, path))
